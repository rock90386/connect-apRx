...
Ptr<NetDevice>
EmuFdNetDeviceHelper::InstallPriv (Ptr<Node> node, Callback<void, Ptr<Packet>> cb) const
{
  Ptr<NetDevice> d = FdNetDeviceHelper::InstallPriv (node);
  Ptr<FdNetDevice> device = d->GetObject<FdNetDevice> ();
  SetFileDescriptor (device);
  device->SetWJReceiveCallback(cb);
  return device;
}
...
EmuFdNetDeviceHelper::SetFileDescriptor (Ptr<FdNetDevice> device) const
{
  ...
  r_addr.sin_addr.s_addr = inet_addr("192.168.3.201");
  r_addr.sin_port = htons(9090);
  ...
  rc = bind (fd, (struct sockaddr *)&r_addr, sizeof (r_addr));
  if (rc == -1)
    {
