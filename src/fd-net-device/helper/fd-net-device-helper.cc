...
NetDeviceContainer
FdNetDeviceHelper::Install (Ptr<Node> node, Callback<void, Ptr<Packet>> cb) const
{
  return NetDeviceContainer (InstallPriv (node, cb));
}
...
Ptr<NetDevice>
FdNetDeviceHelper::InstallPriv (Ptr<Node> node, Callback<void, Ptr<Packet>> cb) const
{
  Ptr<FdNetDevice> device = m_deviceFactory.Create<FdNetDevice> ();
  device->SetAddress (Mac48Address::Allocate ());
  node->AddDevice (device);
