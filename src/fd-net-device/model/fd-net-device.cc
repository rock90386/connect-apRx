...
/*   
  ...
  ser_addr.sin_port = htons(9091);
*/
  socklen_t l = sizeof(ser_addr);

  ssize_t len = recvfrom(m_fd, buf, m_bufferSize, 0, (struct sockaddr *)&ser_addr, &l);
...
FdNetDevice::ForwardUp (void)
{
  ...
  std::cout << isBroadcast << std::endl;
  std::cout << isMulticast << std::endl;
...
FdNetDevice::SendFrom (Ptr<Packet> packet, const Address& src, const Address& dest, uint16_t protocolNumber)
{
  ...
  //std::cout << Simulator::Now() << " In Send From" << std::endl;
  ...
  //cli_addr.sin_addr.s_addr = inet_addr("192.168.162.6");
  //cli_addr.sin_port = htons(9096);

  cli_addr.sin_addr.s_addr = inet_addr("192.168.3.200");
  cli_addr.sin_port = htons(9092);
  //std::cout << "m_fd " << m_fd << std::endl;
  ssize_t written = sendto(m_fd, buffer, len, 0,(struct sockaddr *)&cli_addr , sizeof(cli_addr));
  // std::cout << "12314" << std::endl;
  free (buffer);
  
  if (written == -1 || (size_t) written != len)
    {
      NS_FATAL_ERROR("fuuuuuuuuuuuuuuuuuuck");
      m_macTxDropTrace (packet);
      return false;
    }
...
void
FdNetDevice::SetWJReceiveCallback(Callback<void, Ptr<Packet>> cb)
{
  wjreceiveCallBack = cb;
}
