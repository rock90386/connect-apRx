...
ApWifiMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ApWifiMac")
    ...
    .AddAttribute ("RifsMode", "If non-HT STAs are detected, whether to force RIFS to be disabled within the BSS."
                   ...
                   MakeBooleanChecker ())
    .AddAttribute ("ApMacAddress",
                   "Set Mac address by hand.",
                   Mac48AddressValue (Mac48Address ("00:00:00:00:00:08")),
                   MakeMac48AddressAccessor (&ApWifiMac::GetAddress,
...
ApWifiMac::SetAddress (Mac48Address address)
{
  ...
  std::cout << "SetAddress " << address << std::endl;
  RegularWifiMac::SetAddress (Mac48Address ("00:00:00:00:00:08"));
  RegularWifiMac::SetBssid (Mac48Address ("00:00:00:00:00:08"));
}

// wj add this.
Mac48Address
ApWifiMac::GetAddress (void) const
{
  NS_LOG_FUNCTION (this);
  
  return RegularWifiMac::GetAddress ();
}
...
ApWifiMac::TxFailed (const WifiMacHeader &hdr)
{
  ...
  std::cout << "ApWifiMac::TxFailed " << hdr.IsAssocResp () <<" "<< hdr.IsReassocResp () <<" "<< m_stationManager->IsWaitAssocTxOk (hdr.GetAddr1 ()) << std::endl;
  if ((hdr.IsAssocResp () || hdr.IsReassocResp ())
      && m_stationManager->IsWaitAssocTxOk (hdr.GetAddr1 ()))
    {
  ...
  {
    NS_LOG_FUNCTION ("ApWifiMac::TxFailed, else");
  }
}
