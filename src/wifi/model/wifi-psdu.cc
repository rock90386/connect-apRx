...
WifiPsdu::WifiPsdu (std::vector<Ptr<WifiMacQueueItem>> mpduList, bool s)
  : m_isSingle (s),
    m_mpduList (mpduList)
{
  NS_ABORT_MSG_IF (mpduList.empty (), "Cannot initialize a WifiPsdu with an empty MPDU list");
...
{
  //std::cout << "psdu " << m_mpduList.size () << " " << m_isSingle << std::endl;
...
uint8_t*
WifiPsdu::WjPacketHeaderSerialize(size_t &serialize_size)
{
  int i = 0;
  
  size_t total_len = 0;
  std::vector<Ptr<Packet>> pkt_vec;
  struct Pkt_Info pkt_Info;

  for (i = 0 ; i < (int)m_mpduList.size () ; i++)
  {
    //std::cout << "wj1234" << std::endl;
    Ptr<Packet> pkt = m_mpduList.at (i)->GetPacket ()->Copy ();
    pkt->AddHeader (m_mpduList.at (i)->GetHeader ());

    pkt_Info.m_timestamp = m_mpduList.at (i)->GetTimeStamp();
    pkt_Info.pkt_len = (size_t) pkt->GetSize ();
    pkt_Info_vec.push_back(pkt_Info);

    total_len += (size_t) pkt->GetSize ();
    pkt_vec.push_back (pkt);
  }
  // std::cout << "psdu : " << pkt_Info_vec.size() << std::endl;
  uint8_t *buffer = (uint8_t*)malloc (sizeof(uint8_t)*total_len);
  size_t tmp = 0;
  serialize_size = total_len;
  for (i = 0 ; i < (int)pkt_vec.size () ; i++)
  {
    //std::cout << "wj2234" << std::endl;
    size_t len = (size_t)pkt_vec.at (i)-> GetSize();
    uint8_t *buf = (uint8_t*)malloc (sizeof(uint8_t)*len);
    pkt_vec.at (i)-> CopyData (buf, len);
    std::copy (buf, buf+len, buffer+tmp);
    tmp += len;
    free(buf);
  }
  // std::cout << "pkt_len" << total_len << std::endl;
  return buffer;
}
