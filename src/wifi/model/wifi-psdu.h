...
struct Pkt_Info
{
    ...
    // struct member function : serialize and deserialize method
    std::string serialize()
    {
      int len = (int)pkt_len;
      double time = m_timestamp.GetDouble();
...
public:
  ...
  WifiPsdu (std::vector<Ptr<WifiMacQueueItem>> mpduList, bool s);

  virtual ~WifiPsdu ();

  friend class WjPackage;
  ...
  uint8_t* WjPacketHeaderSerialize (size_t &serialize_size);
...
private:
  ...
  std::vector<struct Pkt_Info> pkt_Info_vec;
};
